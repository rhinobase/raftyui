...
import { useFieldControl } from "@rhinobase/ui";

type CheckboxProps = { children?: React.ReactNode } & ComponentProps<typeof CheckboxPremitive["Root"]>;
// CheckBox Component (Indicator inside the CheckBox Component)
export const Checkbox = forwardRef<HTMLButtonElement, CheckboxProps & { size?: "sm" | "md" | "lg" }>(
      ...
      <div className="flex items-center gap-2">
        <CheckboxPremitive.Root
          ...
          <CheckboxPremitive.Indicator
            className={classNames("dark:text-secondary-100 flex items-center gap-3", className)}>
            <HiCheck
              className={classNames(
                size == "sm" && "h-2.5",
                size == "md" && "h-3",
