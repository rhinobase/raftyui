...
>(({ className, size = "md", ...props }, ref) => (
  <SwitchPrimitives.Root
    className={classNames(
      switchClasses.size[size],
      "peer inline-flex h-[24px] w-[44px] shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:bg-secondary-100 data-[state=unchecked]:bg-input",
      className
    )}
    {...props}
    ref={ref}
  >
    <SwitchPrimitives.Thumb
      className={classNames(
        switchClasses.size[size],
        "pointer-events-none block rounded-full data-[state=checked]:bg-primary-500 shadow-lg ring-0 transition-transform  data-[state=unchecked]:translate-x-0"
      )}
    />
  </SwitchPrimitives.Root>
));
